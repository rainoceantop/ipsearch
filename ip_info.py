...
def tryAgain(ip, ip_info_container, useragents, proxies):
    ...
    try:
        # 尝试获取，如果时间超过0.4秒，返回True重新获取
        ipinfo = requests.get(url, headers=headers,
                              proxies=proxy,  timeout=0.4)
    ...
    if ipinfo['code'] == 1:
        ...
        ip_info_container[ip] = ipinfo['data']
        print('目前容器的长度是：({}/8233)'.format(len(ip_info_container)), flush=True)
        return False


def getIpInfo(ip, ip_info_container, useragents, proxies):
    url = 'http://ip.taobao.com/service/getIpInfo.php?ip=' + ip
    proxy = random.choice(proxies)
    print('尝试用代理{}获取数据'.format(proxy))
    headers = {
    ...
    try:
        ipinfo = requests.get(url, headers=headers, proxies=proxy, timeout=0.4)
    except requests.exceptions.ConnectTimeout:
        return True
    except requests.exceptions.Timeout:
        return True
    ...
    if not ipinfo:
        # 如果无返回数据，重新请求
        while tryAgain(ip, ip_info_container, useragents, proxies):
            time.sleep(0.5)
...
def main():
    ...
    for index, ip in enumerate(ipkeys):
        ...
        p.apply_async(getIpInfo, args=(
            ip, ip_info_container, useragents, proxies,))
        time.sleep(0.8)
        rest += 1
        if rest == 20:
            time.sleep(20)
    ...
    with open(workpath + 'ip_info.json', 'w+') as i:
        i.write(json.dumps(dict(ip_info_container), ensure_ascii=False))
